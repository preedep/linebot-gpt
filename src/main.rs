...
use opentelemetry::{global, Context,KeyValue,metrics::Unit};
...
async fn main() -> std::io::Result<()> {
    ...
    let exporter = opentelemetry_application_insights::Exporter::new(instrumentation_key.to_owned(), ())
        .with_temporality_selector(temporality_selector.clone())
        .with_endpoint(instrumentation_endpoint.to_owned().as_str()).expect("Export exporter error");
    ...
    let cpu_utilization_gauge = meter.to_owned()
    ...
    let chat_gpt_api_key: &str = &env::var("CHATGPT_API_KEY").expect("Failed getting CHATGPT_API_KEY");

    let line_chat_prompt: &str = &env::var("LINE_CHAT_PROMPT").expect("Failed getting LINE_CHAT_PROMPT");
